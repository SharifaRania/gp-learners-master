...
public class SRCuda extends FitnessFunction {
	...
	public void evalPop(Population pop) {
        ...
        public void evaluatePopulationGPU(Population pop) throws IOException{
	        ...
	        //// SpRCo // Prepare to store predicted values for Spearman's rank correlation
    	    double[][] predictedValues = new double[numberOfIndividuals][numberOfFitnessCases];

	        // Store predicted values and set fitness for each individual
            ...
            for(int i=0;i<numberOfIndividuals;i++){
	            ...
	            ////SpRCo. // Assuming fitnessArrayList contains the predicted values in suitable format
                for (int j = 0; j < numberOfFitnessCases; j++) {
            	    predictedValues[i][j] = fitnessArrayList.get(i * numberOfFitnessCases + j);
                }
            }
            
            //// SpRCo // Calculate and set Spearman's rank correlation for each individual
            double[] targetAux = getTargetValues(); // Implement this method to get the target values
            for (int i = 0; i < numberOfIndividuals; i++) {
        		Individual ind = pop.get(i);
        		double spearmanCorrelation = SpearmansRank.calculateSpearmanRankCorrelation(predictedValues[i], targetAux);
        		ind.setFitness("SpearmanCorrelation", 1.0 - spearmanCorrelation); // Assuming fitness is a minimization objective
	        }
        }

        //// SpRCo // Implement this method to return the target values
        private double[] getTargetValues() {
            // You need to implement this method based on how you store and access target values
            return new double[numberOfFitnessCases]; // Placeholder implementation
        }
